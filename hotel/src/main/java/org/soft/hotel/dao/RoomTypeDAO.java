...
public interface RoomTypeDAO {
	...
	@Insert("insert into roomType(roomTypeName,roomTypeStatus,roomTypeTimingMode,roomTypeArea,"
			+ "listPrice,individualPrice,vipPrice,reservationForegift)"
			+ " value(#{roomTypeName},#{roomTypeStatus},#{roomTypeTimingMode},#{roomTypeArea},"
			+ "#{listPrice},#{individualPrice},#{individualPrice},#{reservationForegift})")
	...
	@Update("update roomType set roomTypeName=#{roomTypeName},"
			...
			+ "individualPrice=#{individualPrice},reservationForegift=#{reservationForegift}"
	 ...
	 * 所有干净且空闲的房间
	 */
	@Select("select * from room where roomSanitation=0 and roomStatus=0 and roomTypeId = #{roomTypeId}")
	public List<Room> selectNullroom(int roomTypeId);
