...
public interface ReservationDAO {
	...
	public Users selectVipUser(String userPhone);
	@Select("select * from client where clientPhone = #{clientPhone}")
	public Client selectNoVipUser(String userPhone);
	...
	@Insert("insert into reservation(roomId,roomTypeId,staffId,reservationName,reservationIdCard,reservationPhoneNumber,"
			+ "reservationTime,expectedTimeOfArrival,estimatedTimeOfDeparture,reservationOccupancyDays,reservationStatus,reservationForegift)"
			+ "value(#{roomId},#{roomTypeId},#{staffId},#{reservationName},#{reservationIdCard},#{reservationPhoneNumber},"
			+ "now(),#{expectedTimeOfArrival},#{estimatedTimeOfDeparture},#{reservationOccupancyDays},#{reservationStatus},#{reservationForegift})")
	...
	public RoomType selectRoomtypeInfo(int RoomTypeId);


	/**
	 * 当前预定信息
	 */
	@Select("select * from reservation where reservationPhoneNumber=#{reservationPhoneNumber} order by reservationId desc limit 1")
	public Reservation selectReservationInfo(String reservationPhoneNumber);

	/**
	 * 查看干净且空闲的房间
	 */
	@Select("select * from room where roomTypeId = #{roomTypeId} and roomSanitation=0 and roomStatus=0")
	public List<Room> selectRoomInfo(int roomTypeId);
	 ...
	 * @return
	 */
	@Update("update reservation set unsubscribeTime=now(),reservationStatus =#{reservationStatus},"
			+ "unsubscribeForegift = #{unsubscribeForegift} "
